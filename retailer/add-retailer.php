...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (empty($_POST["retailerIC"])) {
        ...
        if($retailerIC = strlen(trim($_POST["retailerIC"])) < 12)
    ...
    if(empty($input_retailerAddress)){
       ...
       var_dump($retailerName,$retailerEmail,$retailerPhone, $retailerAddress,$retailerPhone);
    ...
    if(empty($retailerName_err) && empty($retailerEmail_err) && empty($retailerIC_err) && empty($retailerAddress_err ) && empty($retailerPhone_err ) ){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            $param_retailerIC = password_hash($retailerIC, PASSWORD_DEFAULT); // Creates a password hash
