...
//$itemName = $S = $M = $L  = $itemDesc  =  $itemPrice =   "";
 ...
 $bulkItemName = $bulkItemGrade = $bulkItemPrice = $bulkItemQtty  = $bulkItemDesc  =  "";
 $bulkItemName_err  = $bulkItemGrade_err  = $bulkItemPrice_err  = $bulkItemQtty_err   = $bulkItemDesc_err    = "";
...
if(isset($_POST["bulkItemID"]) && !empty($_POST["bulkItemID"])){
    ...
    // Validate grade
    $input_bulkItemGrade = trim($_POST["bulkItemGrade"]);
    if(empty($input_bulkItemGrade)){
        $bulkItemGrade_err = "Choose The Grade.";     
    } else{
      // $int_value = (int) $string;
        $bulkItemGrade =  $input_bulkItemGrade;
    }    
    ...
    if( empty($bulkItemName_err) && empty($bulkItemGrade_err) && empty($bulkItemPrice_err) && empty($bulkItemQtty_err )&& empty($bulkItemDesc_err )){
        // Prepare an update statement
        $sql = "UPDATE bulkitem SET bulkItemName=?, bulkItemGrade=?, bulkItemPrice=?, bulkItemQtty=?, bulkItemDesc=? WHERE bulkItemID=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssdiss",  $param_bulkItemName , $param_bulkItemGrade , $param_bulkItemPrice , $param_bulkItemQtty , $param_bulkItemDesc , $param_bulkItemID );
            
            // Set parameters
            $param_bulkItemName = $bulkItemName;
            $param_bulkItemGrade = $bulkItemGrade;
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
                    ...
                    $bulkItemGrade = $row["bulkItemGrade"];
            ...
            <div class="row">
                ...
                <a class="btn btn-primary" href="list-item.php">Back</a>
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group">
                            <label>Grade</label>
                            <select name="bulkItemGrade" <?php echo (!empty($bulkItemGrade_err)) ? 'is-invalid' : ''; ?> id="bulkItemGrade">
                                    <option value="A"> <?php  echo $row ['bulkItemGrade'];?></option>
                            ...
                            <span class="invalid-feedback"><?php echo $bulkItemGrade_err;?></span>
                        </div>
                        <div class="form-group">
                            <label>Price:</label>
